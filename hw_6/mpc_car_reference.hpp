...
class MpcCar {
 private:
  ...
  void linearization(const double& phi,
    ...
    VectorG gc_;
    Ac_.setZero();
    Ac_.coeffRef(0, 2) = - v * sin(phi);
    Ac_.coeffRef(0, 3) = cos(phi);
    Ac_.coeffRef(1, 2) = v * cos(phi);
    Ac_.coeffRef(1, 3) = sin(phi);
    Ac_.coeffRef(2, 3) = tan(delta) / ll_;

    Ad_ = Eigen::Matrix4d::Identity() + dt_ * Ac_;
    //std::cout <<"The Ac is :\n"<<Ac_<<std::endl;

    Bc_.setZero();
    Bc_.coeffRef(2,1) = v / (ll_ * cos( delta ) * cos( delta ));
    Bc_.coeffRef(3,0) = 1.0f;
    ...
    gc_.x() =  v * phi * sin(phi);
    gc_.y() = -v * phi * cos(phi);
    gc_.z() = -(v * delta) / (ll_ * cos(delta) * cos(delta));
    gc_.w() = 0;
    gd_ = dt_ * gc_;
    //std::cout <<"The gd is :\n"<<gd_<<std::endl;  
 ...
 public:
  MpcCar(ros::NodeHandle& nh) : nh_(nh) {
    ...
    nh.getParam("track_points_x", track_points_x); // 获取参考轨迹,为螺旋线
    nh.getParam("track_points_y", track_points_y); 
    nh.getParam("desired_v", desired_v_);  //获取参考速度 0.8m/s
    s_.setWayPoints(track_points_x, track_points_y); 
    // load parameters
    nh.getParam("ll", ll_); // 车长1.282m
    nh.getParam("dt", dt_); // 控制间隔 0.05s 
    nh.getParam("rho", rho_); //
    nh.getParam("N", N_); //horizon 40s? or 40*0.05s
    ...
    for (int i = 0; i < N_; ++i) {
      ...
      Cu_.coeffRef(i * 3 + 0, i * m + 0) = 1.0;
      lu_.coeffRef(i * 3 + 0, 0) = -a_max_;
      uu_.coeffRef(i * 3 + 0, 0) = a_max_;

      // // -delta_max <= delta <= delta_max
       Cu_.coeffRef(i * 3 + 1, i * m + 1) = 1.0;
       lu_.coeffRef(i * 3 + 1, 0) = -delta_max_;
       uu_.coeffRef(i * 3 + 1, 0) = delta_max_;

      // -ddelta_max <= ddelta <= ddelta_max
      if(i == 0){
        Cu_.coeffRef(i * 3 + 2, i * m + 1) = 1.0;
        u_.coeffRef(i * 3 + 2, 0) = -ddelta_max_ ;
        uu_.coeffRef(i * 3 +2, 0) = ddelta_max_; 
        std::cout<<"The cu is :" << Cu_.block((i) * 3 + 0, 0, 3, 2*m)<<std::endl;
      }else{
          Cu_.coeffRef(i * 3 + 2, (i - 1) * m + 1) = -1.0;
          Cu_.coeffRef(i * 3 + 2, i * m + 1) = 1.0;
         std::cout << "The cu is :"<<Cu_.block((i) * 3 + 0, (i - 1) * m + 0, 3, 2*m)<<std::endl;
      }
      ...
      //std::cout<< "The lu_ is :"<< lu_.block(i*3, 0,)
      // // ...
      ...
      //std::cout << "The cx is :"<<Cx_.block(i , i * n, 1, n)<<std::endl;
  ...
  int solveQP(const VectorX& x0_observe) {
    ...
    for (int i = 0; i < N_; ++i) {
      ...
      if (i == 0) {
        ...
        //std::cout << "gg is :" << gg.block(i * n, 0 , n, 1) <<std::endl;
        ...
        //if(i == 1)
        // std::cout << "BB is :" << BB.block(i * n, 0, n, (i +1) * m) <<std::endl;
      ...
      //std::cout << "testx is :" << qx.block(i * n, 0, n, 1) <<std::endl;
    ...
    P_ = BBT_sparse * Qx_ * BB_sparse; // Get 
    q_ = BBT_sparse * Qx_.transpose() * (AA_sparse * x0_sparse + gg_sparse) + BBT_sparse * qx;  //Get
    // osqp
    ...
    lu_.coeffRef(2, 0) = -ddelta_max_ + predictInput_[0].y();
    uu_.coeffRef(2, 0) = ddelta_max_ + predictInput_[0].y();
    return ret;
  }
