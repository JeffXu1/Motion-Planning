            ...
            const Eigen::VectorXd &Time)          // time allocation in each segment
{
    ...
    cout << "n segment size is " << n_segment << endl;
    MatrixXd PolyCoeff = MatrixXd::Zero(n_segment, 3 * p_num1d);           // position(x,y,z), so we need (2 * p_num1d) coefficients
    MatrixXd PolyCoeff_click = MatrixXd::Zero(p_num1d*n_segment,3);
    //MatrixXd PolyCoeff_test = MatrixXd::Zero(p_num1d*n_segment,3);      
    ...
    cout << "Path value: " << endl << Path << endl;
    MatrixXd Path_t = Path.transpose();
    cout << "Path t value: " << endl << Path_t << endl;
    // STEP 3: compute dF for x, y, z respectively
    for(int dim = 0; dim < 3; dim++){
        VectorXd wayPoints = Path_t.col(dim);
        ...
        //cout<< "Here is 200 " << endl;

        cout << "PolyCoeff.block(0, dim*p_num1d, n_segment, p_num1d) :" << endl
            << PolyCoeff.block(0, dim*p_num1d, n_segment, p_num1d) << endl;
        //cout<< "Here/usr/include/eigen3/Eigen/src/Core/CommaInitializer.h:114: Eigen::CommaInitializer<MatrixType>::~CommaInitializer() [with XprType = Eigen::Matrix<double, -1, -1>]: Assertion `(m_row+m_currentBlockRows) == m_xpr.rows() && m_col == m_xpr.cols() && "Too few coefficients passed to comma initializer (operator<<)"' failed. is 110 " << endl;
        MatrixXd poly_coef_1d_t = poly_coef_1d.transpose();
        cout << "poly_coef_1d:" << endl << poly_coef_1d << endl;
        cout << "poly_coef_1d.transpose() :" << endl << poly_coef_1d_t << endl;
        // PolyCoeff(n_segment, 3 * p_num1d)
        // coefficientMatrix = Eigen::MatrixXd::Zero(6 * pieceNum, 3) PolyCoeff_click = MatrixXd::Zero(p_num1d*n_segment,3)
        for(int k = 0; k < n_segment; k++){
            PolyCoeff.block(k, dim*p_num1d, 1, p_num1d) = poly_coef_1d_t.block(0,k*p_num1d, 1, p_num1d);
            PolyCoeff_click.block(k*p_num1d, dim, p_num1d, 1) = poly_coef_1d.block(k*p_num1d,0, p_num1d, 1);
        }

    }
    cout << "PolyCoeff :" << endl << PolyCoeff << endl;

    cout << "PolyCoeff click :" << endl << PolyCoeff_click << endl;
    //cout<< "Here is 300 " << endl;
    //cout<< "PolyCoef size " << PolyCoeff.size()<<endl;
    //PolyCoeff.resize(6 * n_segment, 3);
    //cout<< "PolyCoef resize " << PolyCoeff.size()<<endl;
    //cout<< "Here is 300 " << endl;
    return PolyCoeff_click;
}
