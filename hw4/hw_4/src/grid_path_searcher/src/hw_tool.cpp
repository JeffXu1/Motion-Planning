...
double Homeworktool::OptimalBVP(Eigen::Vector3d _start_position,Eigen::Vector3d _start_velocity,Eigen::Vector3d _target_position)
{
    ...
    */
    ...
    // argmin J funtion T^4 - 4*(vx0^2 + vy0^2 + vz0^2) * T^2 + 24 * (dx * vx0 + dy * vy0 + dz * vz0) * T - 36 * (dx^2 + dy^2 +dz^2)
    // argmin J funtion is sovled by sympy 
    ...
    std::vector<double> j; //Target function
    std::vector<double> t;
    ...
    for(int i = 0; i < 4; i++){ // loop through four roots
        ...
        for(int k = 0; k < 3; k++){ // loop through three dimensions
            a(k) = -12.0 / std::pow(t[i],3) * (_target_position(k) - _start_position(k) - _start_velocity(k) * t[i]) + 6.0 / std::pow(t[i],2) * (-_start_velocity(k));
            b(k) = 6.0 / std::pow(_target_position(k) - _start_position(k) - _start_velocity(k) * t[i],2) - 2.0 / t[i] *(-_start_velocity(k));
